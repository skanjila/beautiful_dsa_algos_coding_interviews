...
from machine_learning_engineering.practical_question_bank import (
...
def test_every_entry_has_required_content():
    for entry in PRACTICAL_QUESTION_BANK:
        ...
        assert len(entry.modeling_approach) > 30
        assert len(entry.system_approach) > 30
...
def test_category_listing_and_filtering():
    ...
    items = filter_by_category("ranking")
    assert items
    assert all(item.category == "ranking" for item in items)
...
def test_search_questions():
    results = search_questions("latency")
    assert results
    assert any(
        "latency" in item.interview_approach.lower()
        or "latency" in item.system_approach.lower()
        or "latency" in item.deep_dive.lower()
        for item in results
    )


def test_get_question_by_slug():
    item = get_question_by_slug("design-search-ranking")
...
def test_render_markdown_contains_expected_sections():
    ...
    assert markdown.startswith("# Machine Learning Engineering Practical Interview Problems")
    assert "## Ranking" in markdown
    assert "Modeling approach:" in markdown
